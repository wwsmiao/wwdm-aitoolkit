...
export async function GET(request: NextRequest, { params }: { params: Promise<{ jobID: string }> }) {
  ...
  const limit = Math.min(Number(limitParam && !isNaN(Number(limitParam)) ? limitParam : 2000) | 0, 20000);
  const sinceStepParam = url.searchParams.get('since_step');
  const sinceStep = sinceStepParam != null && !isNaN(Number(sinceStepParam)) ? Number(sinceStepParam) : null;
  const strideParam = url.searchParams.get('stride');
  const stride = Math.max(1, Number(strideParam && !isNaN(Number(strideParam)) ? strideParam : 1) | 0);
